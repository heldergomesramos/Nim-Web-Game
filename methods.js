...
module.exports.postRequest = function(json, path, response) {  
    switch(path){
		    ...
		    fs.readFile('players.json', function(err,data) {
		  		if(!err) {
					...
					if(playersInQueue[boardIndex] == null) {
						gameId = crypto.createHash('md5').update("game" + (new Date().getTime())).digest('hex');
						...
						setTimeout(() => updater.start(newGame.rack, newGame.getTurnNick(), gameId), 700);
					}
					response.writeHead(200, configs.headers.plain);
        ...
        case '/leave': { //Not tested yet because game needs to exist
			let nick = json.nick;
			let pass = json.password;
			...
			fs.readFile('players.json', function(err,data) {
				if(!err) {
					...
					if(curGame.winner == null) {
						updater.updateBoard(gameId, curGame.stack, curGame.pieces, curGame.rack, curGame.getTurnNick());
					}
					else {
						updater.broadcastWinner(curGame.winner, gameId);
						writeOnRanking(curGame.winner, curGame.otherNick(curGame.winner), curGame.size);
...
function getGameFromId(gameId) {
	for(let curGame of activeGames)
...
function getMediaType(pathname) {
    ...
    if(pos !== -1) 
       mediaType = mediaTypes[pathname.substring(pos+1)];
