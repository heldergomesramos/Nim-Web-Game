...
window.addEventListener(
  ...
  function () {
    ...
    closeRankingScript.addEventListener(
      "click",
      function () {
        rankingTab.style.display = "none";
        document.getElementById("rankingTable").innerHTML =
          '<tr><th class="tableBorder">Nick</th><th class="tableBorder">Wins</th><th class="tableBorder">Games</th></tr>';
      },
      false
    );

    const closeStatsScript = document.getElementById("closeStatsScript");
    closeStatsScript.addEventListener(
      "click",
      function () {
    ...
    closeLoginScript.addEventListener(
      "click",
      function () {
    ...
    cancelLoginScript.addEventListener(
      "click",
      function () {
        loginForm.style.display = "none";
      },
    ...
    closeRulesScript.addEventListener(
      "click",
      function () {
...
function ballOver(id) {
  if (game.canPlay) game.shakeBalls(id, "_shake.gif");
}

function ballOut(id) {
  if (game.canPlay) game.shakeBalls(id, ".png");
}
...
function startGame() {
  ...
  if (vsAi) {
    ...
    if (loggedIn) {
      ...
      errorMessageElement.textContent =
        "You need to be logged in in order to play against other players.";
    }
  }
}
...
function showRankings(data) {
  for (let i = 0; i < data.ranking.length; i++)
    createTable([
      [
        data.ranking[i]["nick"],
        data.ranking[i]["victories"],
        data.ranking[i]["games"],
      ],
    ]);
}
...
function update() {
  eventSource = new EventSource(
    localTestUrl + "update?nick=" + uname + "&game=" + curGameId
  );
  eventSource.onmessage = function (event) {
    let data = JSON.parse(event.data);
    if (!inGame) {
      if (data.turn != null && data.winner == null) createGame();
      ...
      if (data.winner != undefined) {
        eventSource.close();
        if (data.winner == uname) Game.endGame(false);
        else if (data.winner != null) Game.endGame(true);
...
function createGame() {
  ...
  for (let i = 0; i < boardSize; ++i) {
    ...
    for (let j = i; j >= 0; --j) {
      ...
      ballImg.setAttribute(
        "onmouseover",
        'ballOver("' + ballImg.getAttribute("id") + '")'
      );
      ballImg.setAttribute(
        "onmouseout",
        'ballOut("' + ballImg.getAttribute("id") + '")'
      );
      ballImg.setAttribute(
        "onclick",
        'ballClick("' + ballImg.getAttribute("id") + '")'
      );
  ...
  if (!game.yourTurn) {
    setBoardColor(false);
    game.aiMove();
  } else setBoardColor(true);
}

function setBoardColor(green) {
  let boardDiv = document.getElementById("game");
  green
    ? (boardDiv.style.backgroundColor = COLOR_GREEN_DARK)
    : (boardDiv.style.backgroundColor = COLOR_RED_DARK);
}
...
function showSettings() {
  ...
  if (!loggedIn) document.getElementById("loginButton").style.display = "block";
}
...
function removeGameObjects() {
  for (let i = 0; i < game.size; ++i) {
    for (let j = i; j >= 0; --j) {
      ...
      if (ball != null) ball.parentNode.removeChild(ball);
    }
  }
...
function dec2binFormated(number) {
  ...
  if (bin.length == MAX_BITS) return bin;
  let prefix = "";
  while (prefix.length + bin.length != MAX_BITS) prefix += "0";
  return prefix + bin;
}
...
class Stats {
  ...
  updateText() {
    ...
    document.getElementById("statsWinRate").innerText =
      Math.round(this.winRate * 100) + "%";
    document.getElementById("statsHardestDiff").innerText =
      this.hardestDiff != 0 ? this.hardestDiff : "None";
  }
...
class Game {
  ...
  shakeBalls(id, animation) {
    ...
    for (let i = line; i < this.stack[column]; ++i) {
      document.getElementById("id" + column + i).src =
        "res/img/Ball_creature" + animation;
    }
  }
  ...
  destroyBalls(id) {
    ...
    game.canPlay = false;
    if (!vsAi && this.yourTurn) notify(column, this.stack[column]);
    if (this.remaining <= 0)
      setTimeout(function () {
        Game.endGame(!game.yourTurn);
      }, ANIMATION_DESTROY_TIME);
    else setTimeout(game.changeTurn, ANIMATION_DESTROY_TIME);
  }
  ...
  aiMove() {
    ...
    if (rng < this.difficulty) this.bestMove();
    else this.randomMove();
  }

  randomMove() {
    let column = 0;
    while (game.stack[column] == 0) column = RNG(game.size);
  ...
  getSumWord(argStack) {
    let sum = new Array();
    for (let i = 0; i < MAX_BITS; i++) sum[i] = 0;
  ...
  simulateMove(column, line) {
    ...
    return this.checkForPairBinary(this.getSumWord(hypotheticalBoard))
      ? true
      : false;
  }
  ...
  arrayToString(array) {
    let word = "";
    for (let i = 0; i < array.length; i++) word += array[i];
    return word;
  }

  checkForPairBinary(binary) {
    for (let i = 0; i < binary.length; i++)
      if (binary[i] % 2 != 0) return false;
    return true;
  }
  ...
  static endGame(lost) {
    ...
    if (lost) {
      ...
      setBoardColor(true);
      if (vsAi) playerStats.updateHardestDiff(game.difficulty);
    }
